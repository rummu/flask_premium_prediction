...
@app.route("/")
def home():
     return "<h1 style='color:blue'>Hello pycharm - noor-t! changes: Removed FamilyType</h1>"

# @app.route('/collect_garbage',methods = ['GET','POST'])
# def collect_garbage():
#     gc.collect()
#     return str(f"Garbage collected at {datetime.datetime.now()}")
...
#                raise Exception(f'Error: {e}, Request: {request.form}')
#     return decorator
...
#           encoder_model_nri = pickle.load(f)

# with open('model_nri.pkl', 'rb') as f:
#           model_nri = pickle.load(f)
...
#           one_hot_df.drop(columns=['family_type_Joint Family','family_type_Nuclear Family','family_type_null'],inplace=True)
